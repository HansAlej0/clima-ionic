<template>
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      ...
      <ForecastDays :forecastData="forecastData" />
    </ion-content>
  </ion-page>
</template>

<script setup lang="ts">
import ForecastDays from '@/components/Forecast.vue';
...
import { getWeatherForecastByDays, apiKey } from '@/services/ClimateServices';
...
const onCityChange = async () => {
  ...
  timeoutId = setTimeout(async () => {
    ...
    try {
      if (city.value.trim() !== "") {
        forecastData.value = await getWeatherForecastByDays(`${city.value}&lang=es`, apiKey);
      }
    } catch (error) {
